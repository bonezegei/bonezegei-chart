...
class BonezegeiChart {
  ...
  drawBarChart() {
    var w =
      this.ch_width / this.config.x.length / this.config.dataSet.length - 1.1;
    this.xMultiplier = this.ch_width / this.config.x.length;
    for (var a = 0; a <= this.config.x.length; a++) {
      this.xgrid[a] = w / 2 + this.ch_left + a * this.xMultiplier;
    }
  ...
  drawAreaChart() {
    ...
    for (var a = 0; a < this.config.dataSet.length; a++) {
      this.ctx.beginPath();
      this.ctx.moveTo(this.xgrid[0], top);
      var b = 0;
      for (b = 0; b < this.config.dataSet[a].y.length; b++) {
        this.ctx.lineTo(
          this.xgrid[b],
          top - this.config.dataSet[a].y[b] * this.yMultiplier - absMin
        );
      }
      this.ctx.lineTo(this.xgrid[b - 1], top);
      this.ctx.fillStyle = this.config.dataSet[a].color;
      this.ctx.fill();
    }
  }
  //==================================================================================
